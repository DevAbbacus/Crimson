  ...
  <tr class="hover:bg-gray-100 items-center focus-within:bg-gray-100">
    <td class="border-t px-6 py-4" v-if="columns.includes('icon')">
      ...
      <button class="bg-green-500 view-images" @click="viewImages()">
          View Images ({{ form.images.length }})
        </button>
        <template>
    ...
    <td class="border-t px-6 py-4" v-if="columns.includes('accessory_only')">
      <text-input v-model="form.accessory_only" class="mt-1 block w-auto" />
    </td>
    ...
    >
      <resource-selector
        :loader="loadProducts"
        v-model="form.product_accessories"
        direction="bottom"
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        accessory_only: this.product.accessory_only,
        ...
        product_accessories: this.product.product_accessories,
  ...
  methods: {
    ...
    submit() {
      ...
      if(this.$refs.file && this.$refs.file != 'undefined' && this.$refs.file != undefined){
    ...
    loadSelectedAccessories (accessories) {
      console.info('Accessories Items0000000 :- ', accessories);
      console.info("Accessories Items:- ", this.product);
    },
    remove(selection) {
  ...
  created() {
    console.info("Form data :- ", this.form);
