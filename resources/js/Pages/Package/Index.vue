  ...
  <app-layout :status="crms.status">
    <template #header>
      <div class="bg-white p-4 shadow flex items-center justify-between">
        <h2 class="flex text-xl font-bold sm:text-2xl">Package Editor</h2>
    ...
    <template>
      ...
      <jet-dialog-modal :show="modal.show" @close="closeModal()">
        <template #title>
          <h2>{{ modal.type || '' }} Package</h2>
        </template>
        <template #content>
          <resource-form v-model="form" v-if="modal.type" />
        </template>
        <template #footer>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { text: 'Image', value: 'image', type: 'image', icon: 'icon' },
  ...
  watch: {
    query: {
      handler: delay(
        function (query) {
          this.$inertia.replace(this.route('packages.index', query));
        },
        200,
        'query.handler'
      ),
  ...
  methods: {
    async load() {
      ...
      const url = this.route('packages', { ...params });
    ...
    edit(item) {
      this.form = {
        ...item,
        products: (item.products || []).map(({ id, name }) => ({
          text: name,
          value: id,
        })),
      };
    ...
    delete(item) {
      this.$inertia.delete(
        this.route('packages.destroy', { package: item.id })
      );
    },
    closeModal() {
      this.modal = {
    ...
    submitForm() {
      ...
      switch (this.modal.type) {
        case 'create':
          this.$inertia.post(this.route('packages.store'), form, {
        ...
        case 'edit':
          ...
          this.$inertia.post(
            this.route('packages.update', { package: id }),
            form,
            {
              onFinish: () => this.closeModal(),
            }
