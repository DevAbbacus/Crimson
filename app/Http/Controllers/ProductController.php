...
class ProductController extends Controller
{
    ...
    public function list(IRequest $request, $paginate = true, $locally = false)
    {
        ...
        $result = $result->transform(function ($product) use (&$custom_field_columns) {
            ...
            $productCustomFieldsArr = [];
            if (is_array($product->custom_fields)) {
                $productCustomFieldsArr = $product->custom_fields;
                ksort($productCustomFieldsArr);
            }
            if (isset($productCustomFieldsArr['wp_product_end_date'])) {
                $tempWpProductEndDate = $productCustomFieldsArr['wp_product_end_date'];
                unset($productCustomFieldsArr['wp_product_end_date']);
                $productCustomFieldsArr['wp_product_end_date'] = $tempWpProductEndDate;
            }

            $accessory_only = ($product->accessory_only == 1) ? 'Yes' : 'No' ;
            
                //echo "<pre>";print_r($accessory_only);exit();
            return [
                'id' => $product->id,
                'name' => $product->name,
                ...
                'product_accessories' => $this->serializeProductAccessoriesData($product->productAccessories),
                'p_group' => $groups,
                'custom_fields' => $productCustomFieldsArr,
        ...
        $productCustomFieldsColumnArr = Product::where('user_id',Auth::id())->first()->custom_fields;
        ksort($productCustomFieldsColumnArr);
        if (isset($productCustomFieldsColumnArr['wp_product_end_date'])) {
            $tempWpProductColumnEndDate = $productCustomFieldsColumnArr['wp_product_end_date'];
            unset($productCustomFieldsColumnArr['wp_product_end_date']);
            $productCustomFieldsColumnArr['wp_product_end_date'] = $tempWpProductColumnEndDate;
        }
        $json = [
            'items' => $paginate ? $result['data'] : $result,
            'params' => count($params) > 0 ? $params : null,
            'custom_field_columns' => $productCustomFieldsColumnArr,
    ...
    public function serializeProductAccessoriesData($accessories) {
        $productAccessoriesArr = [];
        if (isset($accessories) && sizeof($accessories) > 0) {
            foreach ($accessories as $accessory) {
                $productAccessoriesArr[] = [
                    'value' => $accessory->related_id,
                    'text' => $accessory->related_name,
