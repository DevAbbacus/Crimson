...
class ProductController extends Controller
{
    ...
    public function update(IRequest $request, Product $product)
    {
        ...
        $product->alternativeProducts()->delete();
        foreach ($aIds as $id) {
        ...
        if ($updated && $product->crms_id) {
            try {
                ...
                $input['product_group_id'] = (int) ($productGroup && $productGroup->crms_id);
                // dd($input);
                unset($input['image']);
                $body = ['product' => $input];
    ...
    public function list(IRequest $request, $paginate = true, $locally = false)
    {
        ...
        if($params){
            if(isset($params['blanks'])){
                $blank  = $params['blanks'];
                $blankProductFields = array_diff($params['blanks'], $this->customFields);
                $blankCustomFields  = array_diff($params['blanks'], $blankProductFields);
                $params['blanks']   = $blankProductFields;
            }
        }

        $auth_detail = DB::table('users')->where('id',Auth::id())->first();
        $auth_type='';
        if (!empty($auth_detail)) {
            $auth_type  = $auth_detail->sub_domain;
        }

        if ($auth_type == 'desiredeffect') {
            $user_type = 'magento';
        } else {
            $user_type = 'wp';
        }
        ...
        $result = $result->transform(function ($product) use (&$custom_field_columns) {
            ...
            return [
                ...
                'p_group' => $groups,
                'custom_fields' => (isset($custom_fields[0])) ? $custom_fields[0] : $custom_field_columns,
                'rates' => $product->rates,
                'icon' => $icon,
                'images' => $images,
            ];
